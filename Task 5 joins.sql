...
select order_line, customer_id, product_id,sales,quantity from sales
...
select sum(s.sales),avg(s.profit),c.customer_name,c.city,c.region from sales as s
full join customer as c
on s.customer_id= s.customer_id
group by c.customer_name,c.city,c.region
having avg(s.profit)>100
